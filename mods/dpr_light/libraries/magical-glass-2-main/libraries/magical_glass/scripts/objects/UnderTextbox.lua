local UnderTextbox, super = Class(Object, "UnderTextbox")

UnderTextbox.REACTION_X = {
    ["left"] = 70  -38,
 ["leftmid"] = 160 -38,
     ["mid"] = 260 -38,
  ["middle"] = 260 -38,
...
UnderTextbox.REACTION_X_BATTLE = {
    ["left"] = 60  -40,
 ["leftmid"] = 160 -40,
     ["mid"] = 260 -40,
  ["middle"] = 260 -40,
...
UnderTextbox.REACTION_Y_BATTLE = {
      ["top"] = -10 -2,
      ["mid"] =  30 -2,
   ["middle"] =  30 -2,
["bottommid"] =  45 -2,
...
function UnderTextbox:init(x, y, width, height, default_font, default_font_size, battle_box)
    ...
    self.text = UnderDialogueText("", self.text_x, self.text_y, width + self.wrap_add_w, SCREEN_HEIGHT)
...
function UnderTextbox:advance()
    self.text:advance()
end

function UnderTextbox:setSize(w, h)
...
function UnderTextbox:setActor(actor)
...
function UnderTextbox:setFace(face, ox, oy)
...
function UnderTextbox:setFont(font, size)
    if not font then
        self.font = self.default_font
        self.font_size = self.default_font_size
    else
...
function UnderTextbox:setAuto(auto)
    self.text.auto_advance = auto or false
end

function UnderTextbox:setAdvance(advance)
    self.text.can_advance = advance or false
end

function UnderTextbox:setSkippable(skippable)
    self.text.skippable = skippable or false
end

function UnderTextbox:setAlign(align)
    self.text.align = align or "left"
    self:updateTextBounds()
end

function UnderTextbox:setCallback(callback)
    self.advance_callback = callback
    self.text.advance_callback = callback
end

function UnderTextbox:resetReactions()
    self.reactions = {}
...
function UnderTextbox:addReaction(id, text, x, y, face, actor)
    x, y = x or 0, y or 0
...
function UnderTextbox:resetFunctions()
    self.text.functions = {}
end

function UnderTextbox:addFunction(id, func)
    self.text:addFunction(id, func)
end

function UnderTextbox:setText(text, callback)
    for _,reaction in ipairs(self.reaction_instances) do
        reaction:remove()
    end
    self.reaction_instances = {}
...
function UnderTextbox:getText()
    return self.text.text
end

function UnderTextbox:updateTextBounds()
    if self.face.texture then
...
function UnderTextbox:getBorder()
...
function UnderTextbox:getDebugRectangle()
...
function UnderTextbox:isTyping()
    return self.text:isTyping()
end

function UnderTextbox:isDone()
