local UnderDialogueText, super = Class(Text, "UnderDialogueText")

UnderDialogueText.COMMANDS = { "voice", "noskip", "speed", "instant", "stopinstant", "wait", "func", "talk", "sound", "next" }

function UnderDialogueText:init(text, x, y, w, h, options)
    ...
    self.default_voice = options["default_voice"] or "default"
    self.fast_skip = options["fast_skip"] or false
...
function UnderDialogueText:getDebugInfo()
...
function UnderDialogueText:resetState()
    super.resetState(self)
    self.state["typing_sound"] = self.default_voice
end

function UnderDialogueText:processInitialNodes()
...
function UnderDialogueText:setText(text, callback, line_callback)
...
function UnderDialogueText:advance()
...
function UnderDialogueText:update()
    local speed = self.state.speed

    if not OVERLAY_OPEN then
        if self.fast_skip and Input.pressed("menu") then
        ...
        if self.skippable and ((Input.down("cancel") and not self.state.noskip) or
            ((self.fast_skip and Input.down("menu")) and not self.state.noskip)) then
...
function UnderDialogueText:updateTalkSprite(typing)
...
function UnderDialogueText:playTextSound(current_node)
    ...
    local no_sound = {"\n", " "} -- J.A.R.U. said that * doesn't play the sound but i'm like 99.999% sure it does
    ...
    if (self.state.typing_sound ~= nil) and (self.state.typing_sound ~= "") then
        ...
        if Kristal.callEvent("onUTTextSound", self.state.typing_sound, current_node) then
            return
        end
...
function UnderDialogueText:isNodeInstant(node)
...
function UnderDialogueText:isModifier(command)
    return Utils.containsValue(UnderDialogueText.COMMANDS, command) or super.isModifier(self, command)
end

function UnderDialogueText:registerCommand(command, func, options)
    options = options or {}
    super.registerCommand(self, command, func, options)
    self.custom_command_wait[command] = options["instant"] ~= false
end

function UnderDialogueText:processCustomCommand(node, dry)
...
function UnderDialogueText:processModifier(node, dry)
...
function UnderDialogueText:addFunction(id, func)
    if id then
...
function UnderDialogueText:isTyping()
    return self.state.typing
end

function UnderDialogueText:isDone()
    return self.done
end

return UnderDialogueText