local UnderSpeechBubble, super = Class(Object, "UnderSpeechBubble")

function UnderSpeechBubble:init(text, x, y, options, speaker)
    super.init(self, x, y, 0, 0)
    options = options or {}

    self.layer = BATTLE_LAYERS["above_arena"] - 1

    self.text = UnderDialogueText("", 0, 0, 1, 1, {
...
function UnderSpeechBubble:onRemoveFromStage(stage)
...
function UnderSpeechBubble:advance()
    self.text:advance()
end

function UnderSpeechBubble:setText(text, callback, line_callback)
    if self.actor then
...
function UnderSpeechBubble:setAuto(auto)
    self.text.auto_advance = auto or false
end

function UnderSpeechBubble:setAdvance(advance)
    self.text.can_advance = advance or false
end

function UnderSpeechBubble:setSkippable(skippable)
    self.text.skippable = skippable or false
end

function UnderSpeechBubble:setCallback(callback)
    self.advance_callback = callback
    self.text.advance_callback = callback
end

function UnderSpeechBubble:setLineCallback(callback)
    self.line_callback = callback
    self.text.line_callback = callback
end

function UnderSpeechBubble:setRight(right)
    self.right = right
...
function UnderSpeechBubble:isTyping()
    return self.text:isTyping()
end

function UnderSpeechBubble:isDone()
    return self.text:isDone()
end

function UnderSpeechBubble:update()
    super.update(self)
...
function UnderSpeechBubble:getBorder()
...
function UnderSpeechBubble:getDebugRectangle()
...
function UnderSpeechBubble:getSprite(name)
...
function UnderSpeechBubble:getSpriteSize(name)
...
function UnderSpeechBubble:getTailWidth()
    local tail_width, _ = self:getSpriteSize("tail")
    return tail_width
end

function UnderSpeechBubble:updateSize()
    if self.auto then
