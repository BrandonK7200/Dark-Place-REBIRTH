...
function BallJump:draw()
    ...
    if self.state == "INTRO" or self.state == "MAIN" then
        local score = string.format("%08d", self.score)
        if self.req_score > 0 then
            love.graphics.print(score .. " / " .. self.req_score, 6, 0)
        else
            love.graphics.print(score, 6, 0)
        end
        if self.state == "INTRO" or (self.state == "MAIN" and self.state_timer < 5) then
            local alpha = 1
            if self.state == "MAIN" then alpha = math.min(5 - self.state_timer, 1) end
